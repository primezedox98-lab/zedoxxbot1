...
# Changed sort order to -1 (descending) so newer items appear first
folders_col.create_index([("cat", 1), ("parent", 1), ("created_at", -1)])
folders_col.create_index("number", unique=True, sparse=True)
folders_col.create_index("created_at", -1)  # For sorting by newest
...
# =========================
# 📁 FOLDER SYSTEM (WITH WORKING SUBFOLDERS & NEWEST FIRST)
# =========================
class FS:
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        ...
        folder_data = {
            ...
            "parent": parent,
            "number": number,
            "created_at": time.time()  # This ensures newest items have larger timestamps
        }
    ...
    def get(self, cat, parent=None):
        ...
        if parent:
            ...
            query["parent"] = None
        # Changed sort to use created_at in descending order (newest first)
        return list(folders_col.find(query).sort("created_at", -1))
...
# =========================
# 📂 SHOW FOLDERS (FAST - NEWEST FIRST)
# =========================
def get_folders_kb(cat, parent=None, page=0, items_per_page=15):
    data = fs.get(cat, parent)  # This now returns newest first due to index change
...
def open_folder(c):
    ...
    subfolders = fs.get(cat, name)  # This now returns newest first due to index change
...
def set_vip_msg_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("vip_msg", x.text) or bot.send_message(x.from_user.id, "✅ Updated!", reply_markup=admin_menu()))
    bot.answer_callback_query(c.id)
...
def set_welcome_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("welcome", x.text) or bot.send_message(x.from_user.id, "✅ Updated!", reply_markup=admin_menu()))
    bot.answer_callback_query(c.id)
...
def set_reward_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("ref_reward", int(x.text)) or bot.send_message(x.from_user.id, f"✅ Set to {x.text} points!", reply_markup=admin_menu()) if x.text.isdigit() else bot.send_message(x.from_user.id, "❌ Invalid!"))
    bot.answer_callback_query(c.id)
...
def set_ppd_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("points_per_dollar", int(x.text)) or bot.send_message(x.from_user.id, f"✅ Set to {x.text} pts = $1!", reply_markup=admin_menu()) if x.text.isdigit() else bot.send_message(x.from_user.id, "❌ Invalid!"))
    bot.answer_callback_query(c.id)
