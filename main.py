...
# Create indexes for speed (fixed to avoid session parameter error)
try:
    ...
    # Add created_at index for sorting by newest (without session parameter)
    folders_col.create_index("created_at")
...
def get_folders_kb(cat, parent=None, page=0, items_per_page=15):
    data = fs.get(cat, parent)  # This now returns newest first due to sort
...
def open_folder(c):
    ...
    subfolders = fs.get(cat, name)  # This now returns newest first due to sort
